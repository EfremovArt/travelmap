...
echo "<h2>Fixing Google URLs in Database</h2>";

try {
    $pdo = connectToDatabase();
    
    // Find and fix URLs that start with /http
    ...
    echo "<h3>Found " . count($badUrls) . " URLs to fix in users table:</h3>";
    ...
    if (count($badUrls) > 0) {
        ...
        echo "<h3>✓ All URLs fixed in users table!</h3>";
    } else {
        echo "<p>No bad URLs found in users table.</p>";
    }
    
    echo "<br><p><a href='views/posts.php'>Go to Posts page</a></p>";
