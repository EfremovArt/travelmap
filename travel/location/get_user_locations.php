...
try {
    ...
    error_log("Получение локаций для пользователя: $userId");
    
    // Получаем общее количество локаций пользователя
    $stmt = $db->prepare("
    ...
    foreach ($locations as &$location) {
        $stmtPhotos = $db->prepare("
            SELECT id, file_path, title, description, created_at, position
            FROM photos 
        ...
        $stmtPhotos->execute();
        $location['photos'] = $stmtPhotos->fetchAll();
