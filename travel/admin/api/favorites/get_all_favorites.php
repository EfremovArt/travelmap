...
try {
    ...
    if ($type === 'all' || $type === 'photo') {
        ...
        if ($type === 'photo') {
            ...
            // For 'all' type, we'll collect from all three tables
            $sql = "SELECT 
                        f.id,
                        f.user_id as userId,
                        CONCAT(u.first_name, ' ', u.last_name) as userName,
    ...
    // For 'all' type, sort and paginate the combined results
    if ($type === 'all') {
    ...
    foreach ($favorites as &$favorite) {
        $favorite['userImage'] = $favorite['userImage'] ? normalizeImageUrl($favorite['userImage']) : null;
        $favorite['contentPreview'] = $favorite['contentPreview'] ? normalizeImageUrl($favorite['contentPreview']) : null;
    }
